	...
	public Student get(String no) throws Exception {
		...
		try{
			...
			if(rSet.next()) {
				...
				student.setIs_Attend(rSet.getBoolean("is_attend"));
	...
	public boolean save(Student student) throws Exception {
		...
		try{
			...
			if (old == null){
				...
				statement.setBoolean(5, student.is_Attend());
				statement.setString(6, student.getSchool().getSchool_cd());
			} else {
				//学生が存在した場合
				//プリペアードステートメントにUPDATE文をセット
				statement = connection.prepareStatement(
						"update student name=?,ent_year=?,class_num=?,is_attend=?,where student_no=?");
				//プリペアードステートメントに値をバインド
				statement.setString(1, student.getName());
				statement.setInt(2, student.getEnt_Year());
				statement.setString(3, student.getClass_Num());
				statement.setBoolean(4, student.is_Attend());
